...
function shiftInfo(code){return shifts.find(function(s){return shiftCode(s)===code})||{code:code,name:code,short_name:code};}
...
</script></head><body><div class="shell"><aside class="side"><div class="brand"><div class="logo"></div><div><b>devops-worker</b><div class="hint">schedule</div></div></div><nav class="nav"><a href="/">首页</a><a href="/schedule">排班设置</a><a href="/shifts">班次设置</a><a href="/users">用户</a><a href="/approvals">审批</a><a href="/history">历史</a></nav></aside><main class="content"><div class="topline"><div><div class="kicker">devops-worker</div><h1>{{.Title}}</h1></div><div class="version-box"><span class="tag">{{.NowDate}}</span><label class="tag tz-picker">时区<select onchange="changeDisplayTimezone(this.value)">{{range .TimezoneOptions}}<option value="{{.Name}}" {{if eq .Name $.Config.Timezone}}selected{{end}}>{{.Name}}</option>{{end}}</select></label><span class="tag {{if .IsAdmin}}role-admin{{else}}role-user{{end}}">{{if .IsAdmin}}admin{{else}}普通用户{{end}}</span>{{if .IsAdmin}}<a class="tag" href="/logout">退出</a>{{else}}<a class="tag" href="/login">管理员登录</a>{{end}}</div></div>{{if .Message}}<div class="msg">{{.Message}}</div>{{end}}{{if .Error}}<div class="err">{{.Error}}</div>{{end}}{{end}}
...
const usersTemplate = `{{define "users"}}{{template "layout_start" .}}<div class="card"><h2>新增用户</h2><form method="post" action="/users/create" class="row"><input name="name" placeholder="用户名/别名" required><input name="phone" placeholder="电话号码，可为空"><input name="telegram_user_id" placeholder="Telegram User ID，可为空"><button type="submit">新增用户</button></form><p class="hint">普通用户可以新增用户，并修改自己新增的用户；既有/admin 用户只能查看。删除操作仅 admin 可用；第一次删除会先禁用并异步清理未来未通知排班，禁用后再次删除才会彻底移除配置。</p></div><div class="card" style="overflow:auto;max-height:calc(100vh - 180px);margin-top:10px"><table><thead><tr><th>用户名</th><th>电话</th><th>Telegram User ID</th><th>启用</th><th>来源</th><th>操作</th></tr></thead><tbody>{{range .Users}}{{$can := canEditUser . $.IsAdmin}}<tr>{{if $can}}<form method="post" action="/users/update"><input type="hidden" name="id" value="{{.ID}}"><td><input name="name" value="{{.Name}}" required></td><td><input name="phone" value="{{.Phone}}" placeholder="电话号码"></td><td><input name="telegram_user_id" value="{{.TelegramUserID}}"></td><td><select name="enabled"><option value="true" {{if .Enabled}}selected{{end}}>启用</option><option value="false" {{if not .Enabled}}selected{{end}}>禁用</option></select></td><td><span class="tag">{{if .CreatedBy}}{{.CreatedBy}}{{else}}admin{{end}}</span></td><td><button type="submit">保存</button></form>{{if $.IsAdmin}}<form method="post" action="/users/delete" style="display:inline"><input type="hidden" name="id" value="{{.ID}}"><button class="danger" type="submit">{{if .Enabled}}禁用并清理{{else}}彻底删除{{end}}</button></form>{{end}}</td>{{else}}<td><b>{{.Name}}</b></td><td>{{if .Phone}}{{.Phone}}{{else}}-{{end}}</td><td>{{.TelegramUserID}}</td><td>{{if .Enabled}}启用{{else}}禁用{{end}}</td><td><span class="tag">{{if .CreatedBy}}{{.CreatedBy}}{{else}}admin{{end}}</span></td><td><span class="hint">只读</span></td>{{end}}</tr>{{else}}<tr><td colspan="6">暂无用户</td></tr>{{end}}</tbody></table></div>{{template "layout_end" .}}{{end}}`
