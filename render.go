...
func templateGroupName(id string, groups []UserGroup) string {
...
.users-page{height:calc(100vh - 72px);display:grid;grid-template-rows:auto auto minmax(0,1fr);gap:8px;overflow:hidden}.user-compact-grid{display:grid;grid-template-columns:minmax(340px,.85fr) minmax(420px,1.15fr);gap:8px;align-items:stretch}.user-mini-card{background:#fff;border:1px solid #dbe4ef;border-radius:16px;padding:10px 12px;box-shadow:0 10px 26px rgba(15,23,42,.06);overflow:hidden}.user-mini-head{display:flex;align-items:flex-start;justify-content:space-between;gap:10px;margin-bottom:7px}.user-mini-head h2{margin:0;font-size:15px}.user-mini-head .hint{font-size:10px;line-height:1.35}.mini-tabs{display:inline-flex;gap:4px;padding:3px;border:1px solid #dbe4ef;background:#f8fafc;border-radius:12px;align-self:flex-start}.mini-tab{padding:5px 8px;border-radius:9px;background:transparent;color:#475569;border:0;font-size:11px}.mini-tab.active{background:linear-gradient(135deg,#2563eb,#38bdf8);color:#fff}.user-tab-panel{min-height:78px}.user-manage-card{min-width:0}.user-create-form{display:grid;grid-template-columns:repeat(4,minmax(110px,1fr)) auto;gap:6px;align-items:center}.group-create-form{display:grid;grid-template-columns:minmax(150px,.75fr) minmax(180px,1fr) auto;gap:6px;align-items:center}.group-create-form input{min-width:0;height:32px}.user-create-form input{min-width:0;height:32px}.user-import-form{display:grid;grid-template-columns:minmax(170px,230px) minmax(260px,1fr) auto;gap:7px;align-items:end}.user-import-form textarea{width:100%;height:38px;min-height:38px;resize:vertical;border:1px solid #dbe4ef;border-radius:10px;padding:8px;font-size:11px}.user-import-form input[type=file]{height:32px;font-size:11px}.user-list-panel{min-height:0;background:#fff;border:1px solid #dbe4ef;border-radius:16px;padding:10px 12px;box-shadow:0 10px 26px rgba(15,23,42,.06);display:flex;flex-direction:column;overflow:hidden}.user-list-head{display:flex;align-items:flex-start;justify-content:space-between;gap:10px;margin-bottom:8px;flex:0 0 auto}.user-list-head h2{margin:0;font-size:15px}.user-table-scroll{min-height:0;flex:1 1 auto;overflow:auto;border:1px solid #e2e8f0;border-radius:12px}.user-table-scroll table{margin:0}.user-table-scroll thead th{position:sticky;top:0;z-index:2;background:#f8fafc}.user-table-scroll td input,.user-table-scroll td select{height:30px;min-width:86px}.group-list-compact{margin-top:7px;display:grid;grid-template-columns:minmax(0,1fr) auto;gap:5px;align-items:center;max-height:92px;overflow:auto}.group-row{display:grid;grid-template-columns:minmax(100px,1fr) 82px auto;gap:5px;align-items:center}.group-delete{display:inline}.group-list-compact input,.group-list-compact select{height:28px;font-size:11px}.user-op-row{display:flex;gap:5px;align-items:center;flex-wrap:wrap}.user-op-row form{display:inline}.user-note-line{margin:5px 0 0;font-size:10px;color:#64748b;line-height:1.35}.user-status-tag{white-space:nowrap}@media(max-width:1300px){.user-compact-grid{grid-template-columns:1fr}.user-create-form{grid-template-columns:repeat(2,minmax(120px,1fr)) auto}.user-import-form{grid-template-columns:1fr auto}.user-import-form .paste-area{grid-column:1 / -1}.users-page{height:auto;overflow:visible}.user-list-panel{height:calc(100vh - 310px);min-height:360px}}@media(max-width:820px){.user-create-form,.user-import-form{grid-template-columns:1fr}.user-list-panel{height:65vh}.user-mini-head,.user-list-head{flex-direction:column;align-items:flex-start}}
@media(max-width:1100px){body{overflow:auto}.shell{grid-template-columns:1fr;height:auto}.side{height:auto}.content{height:auto;overflow:visible}.workbench{grid-template-columns:1fr;height:auto}.calendar{min-height:620px}.detail-scroll{overflow:auto}}@media(max-height:760px){.day .items .chip:nth-child(n+3){display:none}.day{padding:5px}.calendar{gap:4px}.weekday{padding:2px}.content{padding:9px}.workbench{height:calc(100vh - 58px)}.calendar-title{font-size:15px}.calendar-sub{display:none}.topline{margin-bottom:6px}.item-card{padding:5px}.chip{font-size:8px}.month-matrix th,.month-matrix td{font-size:9px}.matrix-cell{height:22px}.matrix-cell .mini{height:16px;min-width:16px;font-size:8px}}
</style><script>
...
const usersTemplate = `{{define "users"}}{{template "layout_start" .}}<div class="users-page"><div class="user-compact-grid"><section class="user-mini-card user-manage-card"><div class="user-mini-head"><div><h2>用户 / 分组管理</h2><p class="hint">通过切换选择创建用户或维护用户分组，减少空白窗口占用。</p></div><div class="mini-tabs"><button type="button" class="mini-tab active" data-user-tab-btn="user" onclick="setUserManageTab('user')">新增用户</button>{{if .IsAdmin}}<button type="button" class="mini-tab" data-user-tab-btn="group" onclick="setUserManageTab('group')">用户分组</button>{{end}}</div></div><div class="user-tab-panel" data-user-tab-panel="user"><form method="post" action="/users/create" class="user-create-form"><input name="name" placeholder="用户名/别名" required><input name="email" placeholder="邮箱/SSO 邮箱"><input name="phone" placeholder="电话号码"><select name="group_id">{{range .Groups}}{{if .Enabled}}<option value="{{.ID}}" {{if eq .ID "devops"}}selected{{end}}>{{.Name}}</option>{{end}}{{end}}</select><input name="telegram_user_id" placeholder="Telegram User ID"><button type="submit">新增</button></form><p class="user-note-line">默认加入 devops 分组；SSO 自动关联只做完整字符串匹配，并忽略大小写，不做包含或邮箱前缀模糊匹配。</p></div>{{if .IsAdmin}}<div class="user-tab-panel" data-user-tab-panel="group" style="display:none"><form method="post" action="/groups/create" class="group-create-form"><input name="id" placeholder="分组编码(可空)"><input name="name" placeholder="分组名称" required><button type="submit">新增分组</button></form><p class="user-note-line">默认 devops 组启用。禁用分组后，已有用户仍保留归属，但通知会退回独立通知。</p><div class="group-list-compact">{{range .Groups}}<form method="post" action="/groups/update" class="group-row"><input type="hidden" name="id" value="{{.ID}}"><input name="name" value="{{.Name}}"><select name="enabled"><option value="true" {{if .Enabled}}selected{{end}}>启用</option><option value="false" {{if not .Enabled}}selected{{end}}>禁用</option></select><button type="submit" class="secondary">保存</button></form><form method="post" action="/groups/delete" class="group-delete"><input type="hidden" name="id" value="{{.ID}}"><button class="danger" type="submit">{{if .Enabled}}禁用{{else}}彻底删除{{end}}</button></form>{{else}}<p class="hint">暂无分组</p>{{end}}</div></div>{{end}}</section>{{if .IsAdmin}}<section class="user-mini-card import-export-card"><div class="user-mini-head"><div><h2>批量导入 / 一键导出</h2><p class="hint">导入不会删除 CSV 中缺失的现有用户；字段大小写不敏感。</p></div><a class="btn secondary" href="/users/export">导出 CSV</a></div><form method="post" action="/users/import" enctype="multipart/form-data" class="user-import-form"><div><label class="hint">CSV 文件</label><input type="file" name="file" accept=".csv,text/csv" style="width:100%"></div><div class="paste-area"><label class="hint">或粘贴 CSV 内容</label><textarea name="csv_text" placeholder="name,email,phone,group_id,telegram_user_id,enabled,sso_sub,sso_username,sso_email"></textarea></div><button type="submit">批量导入</button></form><p class="user-note-line">匹配优先级：ID、SSO Sub、邮箱、SSO 邮箱、SSO 用户名、用户名；全部为完整字符串匹配。</p></section>{{end}}</div><section class="user-list-panel"><div class="user-list-head"><div><h2>用户详情列表</h2><p class="hint">固定窗口展示，支持上下和横向滚动，便于大量用户浏览维护。</p></div><span class="pill {{if .IsAdmin}}ok{{else}}off{{end}}">{{if .IsAdmin}}超级管理员{{else}}普通用户{{end}}</span></div><div class="user-table-scroll"><table><thead><tr><th>用户名</th><th>邮箱</th><th>电话</th><th>分组</th><th>Telegram User ID</th><th>SSO关联</th><th>启用</th><th>来源</th><th>操作</th></tr></thead><tbody>{{range .Users}}{{$can := canEditUser . $.IsAdmin}}{{$ug := .GroupID}}<tr>{{if $can}}<form method="post" action="/users/update"><input type="hidden" name="id" value="{{.ID}}"><td><input name="name" value="{{.Name}}" required></td><td><input name="email" value="{{.Email}}" placeholder="邮箱"></td><td><input name="phone" value="{{.Phone}}" placeholder="电话号码"></td><td><select name="group_id">{{range $.Groups}}<option value="{{.ID}}" {{if eq .ID $ug}}selected{{end}}>{{.Name}}{{if not .Enabled}}(禁用){{end}}</option>{{end}}</select></td><td><input name="telegram_user_id" value="{{.TelegramUserID}}"></td><td>{{if .SSOSub}}<span class="tag ok user-status-tag">{{if .SSOUsername}}{{.SSOUsername}}{{else}}SSO{{end}}</span>{{else}}<span class="hint">未关联</span>{{end}}</td><td><select name="enabled"><option value="true" {{if .Enabled}}selected{{end}}>启用</option><option value="false" {{if not .Enabled}}selected{{end}}>禁用</option></select></td><td><span class="tag">{{if .CreatedBy}}{{.CreatedBy}}{{else}}admin{{end}}</span></td><td><div class="user-op-row"><button type="submit">保存</button></form>{{if $.IsAdmin}}<form method="post" action="/users/delete"><input type="hidden" name="id" value="{{.ID}}"><button class="danger" type="submit">{{if .Enabled}}禁用并清理{{else}}彻底删除{{end}}</button></form>{{end}}</div></td>{{else}}<td><b>{{.Name}}</b></td><td>{{if .Email}}{{.Email}}{{else}}-{{end}}</td><td>{{if .Phone}}{{.Phone}}{{else}}-{{end}}</td><td>{{groupName .GroupID $.Groups}}</td><td>{{.TelegramUserID}}</td><td>{{if .SSOSub}}<span class="tag ok user-status-tag">{{if .SSOUsername}}{{.SSOUsername}}{{else}}SSO{{end}}</span>{{else}}<span class="hint">未关联</span>{{end}}</td><td>{{if .Enabled}}启用{{else}}禁用{{end}}</td><td><span class="tag">{{if .CreatedBy}}{{.CreatedBy}}{{else}}admin{{end}}</span></td><td><span class="hint">只读</span></td>{{end}}</tr>{{else}}<tr><td colspan="9">暂无用户</td></tr>{{end}}</tbody></table></div></section></div><script>function setUserManageTab(tab){document.querySelectorAll('[data-user-tab-panel]').forEach(function(el){el.style.display=el.getAttribute('data-user-tab-panel')===tab?'block':'none'});document.querySelectorAll('[data-user-tab-btn]').forEach(function(btn){btn.classList.toggle('active',btn.getAttribute('data-user-tab-btn')===tab)});}</script>{{template "layout_end" .}}{{end}}`
