...
const settingsTemplate = `{{define "settings"}}{{template "layout_start" .}}<div class="card" style="overflow:auto;max-height:calc(100vh - 96px)"><div class="row" style="justify-content:space-between;align-items:flex-start"><div><h2>系统设置</h2><p class="hint">SSO 配置保存在数据目录，保存后立即对后续登录生效。普通用户角色可登录但不会获得管理权限；超级管理员角色可审批和管理配置。</p></div><span class="pill ok">仅 admin 可修改</span></div><form method="post" action="/settings/sso" class="grid" style="grid-template-columns:repeat(auto-fit,minmax(280px,1fr));align-items:start"><section class="panel-card"><h3 style="margin:0 0 8px;font-size:15px">Keycloak / OIDC</h3><label class="field-mini"><input type="checkbox" name="enabled" value="true" {{if .SSOSettings.Enabled}}checked{{end}}> 启用 SSO</label><div><label class="hint">Issuer URL</label><input name="issuer_url" value="{{.SSOSettings.IssuerURL}}" placeholder="https://keycloak.example.com/realms/devops" style="width:100%"></div><div><label class="hint">Client ID</label><input name="client_id" value="{{.SSOSettings.ClientID}}" placeholder="devops-worker" style="width:100%"></div><div><label class="hint">Client Secret</label><input name="client_secret" type="password" placeholder="留空则保留旧密钥" style="width:100%"><input type="hidden" name="keep_client_secret" value="true"></div><div><label class="hint">Redirect URL</label><input name="redirect_url" value="{{.SSOSettings.RedirectURL}}" placeholder="https://dwork.abc.om/sso/callback" style="width:100%"></div><div><label class="hint">Scopes</label><input name="scopes" value="{{.SSOSettings.Scopes}}" placeholder="openid profile email" style="width:100%"></div></section><section class="panel-card"><h3 style="margin:0 0 8px;font-size:15px">角色映射</h3><div><label class="hint">超级管理员用户（sub / username / email / name，逗号分隔）</label><textarea name="admin_users" style="width:100%;min-height:76px;border:1px solid #dbe4ef;border-radius:10px;padding:8px">{{join .SSOSettings.AdminUsers}}</textarea></div><div><label class="hint">超级管理员角色（逗号分隔）</label><input name="admin_roles" value="{{join .SSOSettings.AdminRoles}}" placeholder="devops-worker-admin,admin" style="width:100%"></div><div><label class="hint">普通用户角色（逗号分隔；留空将默认允许 user/devops-worker-user）</label><input name="user_roles" value="{{join .SSOSettings.UserRoles}}" placeholder="devops-worker-user,user" style="width:100%"></div><div class="empty-detail"><b>权限说明</b><p class="hint">普通用户：可查看首页、排班设置、历史、审批记录；无删除权限，不能修改 admin/system 创建的用户或班次。超级管理员：admin 权限，可审批生效、修改 SSO、删除/禁用用户和班次。</p></div><div class="row" style="margin-top:10px"><button type="submit">保存并立即生效</button><a class="btn secondary" href="/login">测试登录入口</a></div></section></form></div>{{template "layout_end" .}}{{end}}`
