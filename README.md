...
- 排班设置页和历史查询页支持按月份导出排班表，支持 XLSX/CSV；导出时会根据该月审批历史自动生成版本下拉框，默认导出最新版本，也可导出上个月或任意历史月份的指定版本；导出文件包含姓名、手机号和班次时间说明。
...
| `/schedule` | 排班设置，日历点击多选日期，弹窗选择用户和班次，先加入草稿，最终统一提交 Telegram 审批；超级管理员可在右侧按版本导出本月排班表。 |
| `/shifts` | 班次设置，维护班次编码、名称、简称、开始时间、结束时间、时区和启用状态；跨天自动判断。 |
| `/users` | 用户管理，维护用户名/别名和 Telegram User ID。 |
| `/approvals` | 审批记录。 |
| `/history?date=YYYY-MM-DD` | 以日历页方式查阅历史排班；超级管理员可选择该月任意历史版本并导出月表。 |
| `/schedule/export?year=YYYY&month=M&version=revision:N&format=xlsx` | 排班月表导出接口，`format` 支持 `xlsx` / `csv`，`version` 为空时默认该月最新版本。 |
| `/previews/<approval-id>.html` | 审批预览 HTML。 |


## 排班月表导出

- 在 `/schedule` 和 `/history` 页面，超级管理员会看到“导出版本”下拉框。系统会读取该月份相关的正式排班快照和已审批记录，自动生成可选版本，默认选中该月最新版本。
- 当前月导出默认使用当前生效排班；如果导出上个月或任意历史月份，则默认使用该月能找到的最后一个历史版本。
- XLSX 导出包含三个工作表：`排班月表`（按用户为行、日期为列的月度矩阵，包含姓名、手机号、用户组）、`排班明细`（逐条列出日期、星期、姓名、手机号、班次、班次时间说明、开始/结束时间、Telegram User ID、Staff ID）和 `班次说明`（列出班次编码、名称、简称、开始/结束时间、时区、是否跨天、是否通知）。
- CSV 导出为月度矩阵，包含姓名、手机号、用户组和每天排班简称；文件末尾会追加 `班次时间说明` 区块，便于直接粘贴或再次处理。
- 导出内容包含手机号，因此 Web 路由限制为 `admin` 超级管理员访问；普通用户仍可查看页面和提交审批，但不能下载含手机号的表格。
- 直接调用示例：`/schedule/export?year=2026&month=5&format=xlsx` 默认导出 2026 年 5 月最新版本；`/schedule/export?year=2026&month=5&version=revision:12&format=csv` 导出指定 revision 的 CSV。
...
- 默认每天 `09:00` 按 `APP_TIMEZONE` 时区向 `GROUP_CHAT_IDS` 配置的群发送当天排班明细，标题会展示日期和星期几，并会 @ `APPROVER_USER_IDS` 中配置的一个或多个审批人。
- 日报发送成功后会自动置顶第一段日报消息；如果置顶失败（例如 bot 缺少置顶权限），系统只记录日志，不重复刷屏发送日报。
- 可通过 `DAILY_REPORT_TIME=09:00` 调整发送时间。
- 日报按班次分组，分组顺序为：早班、中班、晚班、正常班、休息、年假、病假、自定义班次。
- 日报里的夜班按“即将开始的夜班”口径展示：如果夜班开始时间早于或等于日报发送时间（默认 `00:00` 夜班，日报 `09:00` 发送），当天日报会排除当天 `00:00` 已开始/已结束的夜班，改为展示次日 `00:00` 开始的夜班，并在日报头部和明细行标注班次日期。
- 每个班次分组会显示班次时间、人数，以及每个用户的通知状态、已读状态、TG 绑定状态和电话资料。
- 年假、病假、事假、调休、补休等休假类班次会在日报明细中补充“剩余休假”信息；当前按该员工同一休假类型的连续已排班区间计算，显示含当天剩余天数/小时和结束日期。
